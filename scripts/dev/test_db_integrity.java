...
public class test_db_integrity {
    public static void main(String[] args) {
        ...
        try (Connection conn = DriverManager.getConnection(jdbcUrl, username, password)) {
            ...
            // 检查用户数据
            checkUserData(conn, passwordEncoder);
            
            // 检查user_profiles表结构
            checkUserProfilesTableStructure(conn);
            
            // 检查user_profiles数据
            checkUserProfilesData(conn, passwordEncoder);
    ...
    private static void checkUserData(Connection conn, BCryptPasswordEncoder passwordEncoder) throws SQLException {
        System.out.println("\n=== 检查用户数据 ===");
        ...
        try (Statement stmt = conn.createStatement();
            ...
            while (rs.next()) {
                ...
                // 测试密码验证
                boolean passwordValid = passwordEncoder.matches("123456", passwordHash);
                System.out.println("  密码 '123456' 验证: " + (passwordValid ? "成功" : "失败"));
            }
        }
    }
    ...
    private static void checkUserProfilesData(Connection conn, BCryptPasswordEncoder passwordEncoder) throws SQLException {
        System.out.println("\n=== 检查user_profiles数据 ===");
